...
async function login(page: import('@playwright/test').Page, email = 'test@example.com') {
  await page.goto('/')
...
test.describe('Upload Recipes', () => {
  ...
  test('can navigate to upload page', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: 'Upload a Recipe' })).toBeVisible()
  })
    ...
    await page.route('**/api/recipes**', async (route) => {
      if (route.request().method() === 'POST') {
        ...
        await route.fulfill({
          ...
          body: JSON.stringify({
            success: true,
            recipe: { uuid: 'test-uuid', name: 'Test', user_uuid: 'user-uuid', tag_string: '', created_at: new Date().toISOString() },
            tags: [],
          }),
        })
      } else {

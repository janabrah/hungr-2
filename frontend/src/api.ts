...
export async function getRecipes(email: Email): Promise<RecipesResponse> {
  const response = await fetch(
    `${API_BASE}/api/recipes?email=${encodeURIComponent(email)}`,
  );
  if (!response.ok) {
...
export async function deleteRecipe(recipeUUID: UUID): Promise<void> {
  const response = await fetch(
    `${API_BASE}/api/recipes?uuid=${encodeURIComponent(recipeUUID)}`,
    {
      method: "DELETE",
    },
  );
...
export async function getRecipeSteps(
  ...
  const response = await fetch(
    `${API_BASE}/api/recipes/${encodeURIComponent(recipeUUID)}/steps`,
  );
  if (!response.ok) {
    throw new Error(
      `Failed to fetch recipe steps: ${response.status.toString()}`,
    );
  }
...
export async function extractRecipeFromURL(
  ...
  if (!response.ok) {
    ...
    throw new Error(
      data.error ?? `Failed to extract recipe: ${response.status.toString()}`,
    );
  }
  return response.json() as Promise<RecipeStepsResponse>;
}
